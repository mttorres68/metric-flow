    ...
    return (
        <div className="flex h-screen bg-slate-50 dark:bg-[var(--background)] overflow-hidden">
            ...
            <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ${isCollapsed ? 'ml-20' : 'ml-60'}`}>
                ...
                {/* Toggle + conteúdo — flex-1 min-h-0 garante que overflow-auto funcione */}
                <div className="flex-1 min-h-0 flex flex-col">
                    ...
                    {/* Scroll container único — mesmo padrão para diária e semanal */}
                ...
                </div>{/* fim flex-1 min-h-0 flex flex-col */}
            </div>
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            >
                ...
                {/* key inclui índice: ids de vendedor podem repetir entre revendas */}
...
// Mapa vendedor × métrica (heatmap) por revenda + análise inteligente (LLM) por
// revenda, com persistência no banco. Reaproveita o EditorAnalise da visão diária
// via setAnalise.

// Rótulos curtos das colunas da matriz (o nome completo vai no tooltip).
...
// Remove qualquer tag fora da allowlist para evitar injeção de <style>/<table> no DOM
function sanitizeInsightHtml(html: string): string {
    return html.replace(/<(?!\/?(?:p|ul|li|strong|br)\b)[^>]+>/gi, "");
}

function RecorrenciaSemanal({ dataInicio, dataFim, revendaFiltro, setAnalise }: {
    dataInicio: string;
    dataFim: string;
    ...
    async function fetchSemanalBase64(rev: string, porRevenda: Record<string, any[]>, insights: Record<string, string>) {
    ...
    async function handleDownloadPDF(rev: string, porRevenda: Record<string, any[]>, insights: Record<string, string>) {
        ...
        try {
            ...
            if (!resp.ok) {
                const err = await resp.json().catch(() => ({}));
                toast.error(err.error ?? "Erro ao gerar PDF semanal.");
                return;
            }
            const blob = await resp.blob();
            const filename =
    ...
    async function fetchUnificadoBase64(revendas: string[], porRevenda: Record<string, any[]>, insights: Record<string, string>) {
    ...
    async function handleDownloadUnificado(revendas: string[], porRevenda: Record<string, any[]>, insights: Record<string, string>) {
        setDownloadingUnificado(true);
        try {
            const { blob, filename } = await fetchUnificadoBase64(revendas, porRevenda, insights);
            const url = URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url; a.download = filename; a.click();
            URL.revokeObjectURL(url);
    ...
    async function handleWAUnificado(revendas: string[], porRevenda: Record<string, any[]>, insights: Record<string, string>) {
        ...
        try {
            const { base64, filename } = await fetchUnificadoBase64(revendas, porRevenda, insights);
    ...
    async function handleEnviarWA(rev: string, porRevenda: Record<string, any[]>, insights: Record<string, string>) {
        if (!waConectado) { toast.error("WhatsApp desconectado."); return; }
        const allDests = destQuery.data ?? [];
        ...
        setSendingWA(rev);
        try {
            const { base64, filename } = await fetchSemanalBase64(rev, porRevenda, insights);
    ...
    // Versão incremental por revenda: muda apenas em updates externos (DB ou LLM),
    // não na digitação do usuário. Força remount do EditorAnalise com o html correto.
    const [insightVersions, setInsightVersions] = useState<Record<string, number>>({});
    const [gerando, setGerando] = useState<string | null>(null);
    const [salvando, setSalvando] = useState<string | null>(null);

    // Reidrata insights já salvos no banco para esta semana
    useEffect(() => {
        ...
        <div className="px-6 pb-6 pt-4 flex flex-col gap-6">
            ...
            {revendas.length > 0 && (
                <div className="flex items-center justify-end gap-2">
                    <button
                        onClick={() => handleDownloadUnificado(revendas, porRevenda, insights)}
                    ...
                    <button
                        onClick={() => handleWAUnificado(revendas, porRevenda, insights)}
                    ...
                    <div key={rev} className="bg-white dark:bg-[var(--card)] rounded-xl border border-slate-100 dark:border-[var(--border)] overflow-hidden">
                        <div className="px-5 py-3 border-b border-indigo-100 dark:border-indigo-900/40 bg-indigo-50/50 dark:bg-indigo-900/10 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    onClick={() => handleDownloadPDF(rev, porRevenda, insights)}
                                ...
                                <button
                                    onClick={() => handleEnviarWA(rev, porRevenda, insights)}
                            ...
                            <table className="w-full text-xs">
                                <thead className="sticky top-0 z-10">
                                    <tr className="bg-green-950 text-slate-100">
                                        ...
                                        {flags.map(f => (
                                            <th key={f.id} title={f.label} className="px-2 py-2 text-center text-[11px] font-bold uppercase tracking-tight leading-tight align-bottom">{FLAG_SHORT[f.id] ?? f.label}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
