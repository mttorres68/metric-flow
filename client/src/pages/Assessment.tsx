...
// ─────────────────────────────────────────────────────────────────────────────
// RankingView — ranking de revendas por score do assessment
// ─────────────────────────────────────────────────────────────────────────────
const MEDAL = ["🥇", "🥈", "🥉"];
...
const DEFAULT_COLOR = { bg: "bg-white dark:bg-[var(--card)]", border: "", badge: "bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400" };

function RankingView({ isDark: _isDark, cardBorder, cardShadow }: {
    isDark: boolean;
    cardBorder: string;
    cardShadow: string;
}) {
    ...
    const mesLabel = MESES_RES.find(m => m.num === selectedMes)?.label ?? "";
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <select value={selectedMes} onChange={e => setSelectedMes(Number(e.target.value))}
                        ...
                        {MESES_RES.map(m => <option key={m.num} value={m.num}>{m.label}</option>)}
                    </select>
                    <select value={selectedAno} onChange={e => setSelectedAno(Number(e.target.value))}
                        className="text-xs border border-slate-200 dark:border-[var(--border)] rounded-lg px-2 py-1.5 bg-white dark:bg-[var(--input)] focus:outline-none focus:ring-2 focus:ring-indigo-200 dark:text-slate-200">
